...
exports.populate = function(S, data, stage, region) {

  // Validate required params
  if (!S || !data || !stage || !region) throw new SError(`Missing required params: Serverless, project, stage, region`);

  let meta      = S.state.getMeta();
  let project   = S.state.getProject();
...
function pathToContext(path) {
  ...
  // replace Windows pathway delimiters
